...
import { Home, Users, User, Shield, Menu, X, LogOut, Bell, Gamepad2 } from "lucide-react";
...
export default function StudentLayout() {
  ...
  const navItems = [
    { to: "/student", icon: <Home size={22} />, label: "Học tập", end: true },
    { to: "/student/learning-path", icon: <Gamepad2 size={22} />, label: "Lộ trình" },
    { to: "/student/groups", icon: <Users size={22} />, label: "Lớp học" },
    { to: "/student/profile", icon: <User size={22} />, label: "Cá nhân" },
  ];
    ...
    <div className="flex h-screen bg-indigo-50/30 font-sans overflow-hidden">
      ...
      >
        <div className="p-6 flex items-center gap-3">
          <div className="w-10 h-10 bg-pink-500 rounded-xl flex items-center justify-center shadow-lg shadow-pink-100 flex-shrink-0">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <header className="hidden lg:flex items-center justify-between px-8 py-4 bg-white/80 backdrop-blur-md border-b border-indigo-50 sticky top-0 z-20">
          ...
          <div className="flex items-center gap-4">
            <button className="p-2.5 text-gray-400 hover:text-pink-500 hover:bg-pink-50 rounded-xl transition-all relative">
              <Bell size={20} />
            ...
            <div className="flex items-center gap-3 pl-2">
              <div className="text-right">
                <p className="text-sm font-black text-gray-800">Minh Đức</p>
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Học sinh Lớp 5A</p>
              </div>
              <div className="w-10 h-10 bg-gradient-to-br from-indigo-100 to-pink-100 rounded-xl flex items-center justify-center text-indigo-600 font-black border border-white shadow-sm">
                MD
              </div>
            </div>
          </div>
        ...
        <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-lg border-t border-pink-100 flex justify-around p-3 z-50 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] rounded-t-3xl">
          {navItems.map((item) => (
            <NavLink 
              key={item.to}
              to={item.to} 
              ...
              className={({ isActive }) => `
                ...
                ${isActive ? 'text-pink-600 scale-110' : 'text-gray-400 opacity-70'}
              `}
            >
              {item.icon}
